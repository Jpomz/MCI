...
Now we will plot the results using ggplot. We will reorder the sites based on their mean.MCI score, from lowest to highest, using the `reorder()` function. We will also add reference lines (`geom_hline()`) delineating which sites are considered excellent, good, fair, and poor. 
...
amdi.surber <- bugs.index %>%
  group_by(site, surber) %>%
  filter(!is.na(AMDI)) %>% # remove taxa that do not have AMDI score
